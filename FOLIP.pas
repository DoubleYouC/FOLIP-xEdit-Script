...
begin
    ...
    try
        ...
        for si := 0 to Pred(ElementCount(substitutions)) do begin
            ...
            if slSubstitutions.IndexOf(originalMat) <> -1 then begin
                //duplicate found
                Remove(sub);
                Inc(count);
...
begin
    for i := Pred(tlEnableParents.Count) downto 0 do begin
        ...
        for pi := Pred(ReferencedByCount(p)) downto 0 do begin
            ...
            bBaseHasLOD := slHasLOD.IndexOf(RecordFormIdFileId(base)) <> -1;
        ...
        if bHasOppositeEnableRefs then begin
            ...
            for oi := Pred(tlOppositeEnableRefs.Count) downto 0 do begin
                ...
                OverOrNew := 'Overrides';
                if slFakeStatics.IndexOf(RecordFormIdFileId(BaseRecord(r))) <> -1 then OverOrNew := 'New';
...
begin
    ...
    if (Signature(s) = 'SCOL') and (cnt > 0) then begin
        ...
        recordId := RecordFormIdFileId(s);
        if slHasLOD.IndexOf(recordId) = -1 then slHasLOD.Add(recordId);
...
begin
    ...
    for i := 0 to Pred(slTopPaths.Count) do begin
        ...
        if (SameText(level, '0') and (slNifFiles.IndexOf(p2) <> -1)) then begin
